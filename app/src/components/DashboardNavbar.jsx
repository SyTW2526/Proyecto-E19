...
const DashboardNavbar = ({ user, setUser }) => {
  ...
  // Determinar si estamos en home o en rutas especiales donde no mostrar menús
  const isHomeRoute = location.pathname === '/';
  const isSpecialRoute = location.pathname === '/perfil' || location.pathname === '/about';
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 bg-white py-2 sm:py-3 lg:py-4 px-3 sm:px-4 lg:pr-8 text-gray-800 flex items-center justify-between z-50">
      ...
      <div className="flex items-center gap-3 sm:gap-6 lg:gap-10 xl:gap-17">
        <button 
          onClick={() => navigate('/')}
          className="flex items-center gap-2 text-base sm:text-lg lg:text-xl font-bold text-gray-800 hover:text-[#7024BB] transition-colors cursor-pointer"
        >
      ...
      {/* Menú de navegación central - solo visible si NO estamos en home */}
      {!isHomeRoute && (
